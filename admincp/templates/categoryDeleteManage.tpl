<div class="page-header">
    <h1 class="title">Manage Deleted Category{if $smarty.request.resid neq ''} - {$resname}{/if}</h1>
      <ol class="breadcrumb">
        <li><a href="index.php">Dashboard</a></li>
        <li class="active">Manage Deleted Category{if $smarty.request.resid neq ''} - {$resname}{/if}</li>
      </ol>
		...
		<div class="panel-body">
			...
			{if $totalRecords gt 0}
				...
				<div class="col-sm-7 no-padding">	
					<form name="categorymanage" method="post" action="categoryDeleteManage.php{if $smarty.request.limit neq ''}?limit={$smarty.request.limit}{/if}" />
                        {if !$smarty.get.resid}
				...
				<div  class="manageButtonLeft">
					...
					<div class="fliterbuttonContShow processButton" id="searchkey" style="display:none;">
						<form name="filterform" method="post" action="categoryDeleteManage.php{if $smarty.request.searchrestaurantid neq ''}?resid={$smarty.request.searchrestaurantid}{/if}" onsubmit="return filterValidation();">
				...
				<div class="tableListContainer table-responsive">
					<table class="table table-bordered table-hover table-striped" {if $smarty.request.resid neq ''}id="table_catgory"{/if}>
						...
						<tr id="{$mainCategory_list[maincat].maincateid}">
							...
							<td align="center" class="">{$mainCategory_list[maincat].addeddate|date_format:"%d - %m - %Y"}</td>
							...
							<td align="center" class="listCont">
								<span class="EditDeleteButton">
									...
									<a href="javascript:;" class="btn btn-icon btn-light" onclick="return restoreProcess('delete','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$mainCategory_list[maincat].maincateid}','delete_category');" >
											<i class="fa fa-recycle"></i>
									</a>
								</span>
								<span class="EditDeleteButton">
										<a href="javascript:;" class="btn btn-light btn-icon btn-sm" onclick="return changeStatus('delete','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$mainCategory_list[maincat].maincateid}');" style="cursor:pointer;" >
