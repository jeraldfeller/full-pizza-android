...
<div class="panel panel-default">
	...
	{if $totalRecords gt 0}
		...
		<div class="col-sm-7 no-padding">	
			...
			<select class="selectpicker" name="sortby" id="sortby" size="1" onchange="document.followersmanage.submit();">
				...
				<optgroup label="Others">
					<option value="casc" {if $smarty.request.sortby eq 'casc'}selected="selected"{/if}>&nbsp;&nbsp;Followers Name A to Z</option>
					<option value="cdesc" {if $smarty.request.sortby eq 'cdesc'}selected="selected"{/if}>&nbsp;&nbsp;Followers Name Z to A</option>
				</optgroup>				
			</select>
            </div>
			</form>
		...
		<div  class="manageButtonLeft">
		    ...
		    <div class="fliterbuttonContShow processButton" id="searchkey" style="display:none;">
	     		<form name="filterform" method="post" action="followersManage.php" onsubmit="return filterValidation();">
				    ...
				    <input type="text" name="keyword" id="keyword" value="{$smarty.request.keyword}" class="textboxFilter" placeholder="Followers Name" />
		...
		<div class="manageButtonLastCont">
			...
			<input type="button" class="btn btn-info btn-sm but_delete" value="Delete" style="display:none;" onclick="adminActivateDeactivate('delete','deletefield','{$whereField}','{$tablename}','{$word}','followers');" />
		...
		<div class="tableListContainer">
			<table class="table table-hover table-bordered table-striped">
				...
				<tr {if $trvar is even}class="listLightGray"{/if}>
					<td >
						<div class="checkbox checkbox-primary no-margin">
							<input type="checkbox" class="case" value="{$followers_List[followers].followers_id}" onclick="individualSelect();" id="tableselect_{$smarty.section.followers.rownum}"/>
							<label for="tableselect_{$smarty.section.followers.rownum}"></label>
						</div>
						</td>
					<td >{$followers_List[followers].sno}</td>
					<td>{$followers_List[followers].followers_name|stripslashes}</td>
					<td >{$followers_List[followers].followers_url|stripslashes}</td>
					{*<td >
						{if $followers_List[followers].followers_logo neq ''}
							<div class="largeImgTooltip">
								<img src="{$SITE_IMAGE_FOLLOWERS_URL}/{$followers_List[followers].followers_logo|stripslashes}" alt="{$followers_List[followers].followers_name|stripslashes}" title="{$followers_List[followers].followers_name|stripslashes}" class="imgborder" />
								<span><img src="{$SITE_IMAGE_FOLLOWERS_URL}/{$followers_List[followers].followers_logo|stripslashes}" alt="{$followers_List[followers].followers_name|stripslashes}" title="{$followers_List[followers].followers_name|stripslashes}" /></span>
							</div>
						{else}
							No Photo
						{/if}
					...
					<td >
						{if $followers_List[followers].followers_header eq 'Yes'}
							<img src="images/star_yellow.png" alt="Header" title="Header" onclick="return changeStatus('No','followers_header','{$whereField}','{$tablename}','{$word}','{$followers_List[followers].followers_id}','{$filename}');" style="cursor:pointer;" />
						{else}
							<img src="images/star_grey.png" alt="Normal" title="Normal" onclick="return changeStatus('Yes','followers_header','{$whereField}','{$tablename}','{$word}','{$followers_List[followers].followers_id}','{$filename}');" style="cursor:pointer;" />
						{/if}
					</td>
                    <td >{$followers_List[followers].followers_id}</td>
					<td >{$followers_List[followers].addeddate|date_format:"%d - %m - %Y"}</td>				
					<td  id="chgstatus{$followers_List[followers].followers_id}">
						{if $followers_List[followers].followers_status eq '1'}
						<img src="images/icon_active.png" alt="Active" title="Active" onclick="return changeStatus('0','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$followers_List[followers].followers_id}','{$filename}');" style="cursor:pointer;" />
						{else}
						<img src="images/delete.png" alt="Deactive" title="Deactive" onclick="return changeStatus('1','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$followers_List[followers].followers_id}','{$filename}');" style="cursor:pointer;" />
						{/if}
					</td>
                    {if $VIEWABLE eq 'Yes'}
					<td >
						<span class="EditDeleteButton">
							<a href="followersAddEdit.php?cusid={$followers_List[followers].followers_id}" class="btn btn-light btn-icon btn-sm" >
								<i class="fa fa-pencil"></i>
							</a>
						</span>
						<span class="EditDeleteButton">
							<img src="images/icon_delete.png" width="14" height="14" alt="Delete" title="Delete" onclick="return changeStatus('delete','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$followers_List[followers].followers_id}','followers');" style="cursor:pointer;" />
						</span>
					</td>
                    {/if}
				</tr>
