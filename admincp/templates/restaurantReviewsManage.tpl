<div class="page-header">
    <h1 class="title">Manage Reviews {if $smarty.get.resid} - {$restname}{/if}</h1>
      <ol class="breadcrumb">
        <li><a href="index.php">Dashboard</a></li>
        <li class="active">Manage Reviews {if $smarty.get.resid} - {$restname}{/if}</li>
      </ol>
...
<div class="panel panel-default">
	<div class="panel-body">
		...
		<div  class="filterbutton">
			<span class="topName1">Total Records:</span> <span class="topName2">{$reviewsCnt}</span>
			<span class="topName1">Active Records:</span> <span class="topName2">{$activerec}</span>
			<span class="topName1">Deactive Records:</span> <span class="topName2">{$deactiverec}</span>
		</div>
		<!-- Sort By -->
		...
		<div class="col-sm-7 no-padding">	
			<form name="reviewmanage" method="post" action="restaurantReviewsManage.php{if $smarty.get.resid neq ''}?resid={$smarty.get.resid}{/if}" />
			...
			<select class="selectpicker"  name="sortby" id="sortby" size="1" onchange="document.reviewmanage.submit();">
				...
				<optgroup label="Others">
					...
					<option value="tasc" {if $smarty.request.sortby eq 'rasc'}selected="selected"{/if}>&nbsp;&nbsp;Rating 0 to 9</option>
					<option value="tdesc" {if $smarty.request.sortby eq 'rdesc'}selected="selected"{/if}>&nbsp;&nbsp;Rating 9 to 0</option>
				</optgroup>				
			</select>
            </div>
			</form>
	       ...
	       <div class="btn btn-success btn-sm" onclick="return filterOptShowHide();">
			...
			<div class="fliterbuttonContShow processButton" id="searchkey" style="display:none;">
				<form name="filterform" method="post" action="restaurantReviewsManage.php" onsubmit="return filterValidation();">
					...
					<input type="text" name="keyword" id="keyword" value="{$smarty.request.keyword}" class="textboxFilter" placeholder="Customer/Restaurant Name" />
			...
			<div class="fliterbuttonContShow processButton" id="export" style="display:none;">
				<form name="exportform" method="post" onsubmit="return exportValidation();">
					...
					<select class="restManageNameDrop" name="exportfile" id="exportfile">				 	 
			...
			<div class="fliterbuttonContShow processButton" id="import" style="display:none;">
				<form name="importform" method="post"  enctype="multipart/form-data" onsubmit="return importValidation();" >
					<input type="hidden" name="action" value="importProcess" />	
					 ...
					 <input class="fileUpload" type="file" name="importfile" id="importfile" />
		...
		<div class="manageButtonLastCont">
			...
			<input type="button"  class="btn btn-default btn-sm but_activate" value="Activate" onclick="adminActivateDeactivate('1','{$fieldname}','{$whereField}','{$tablename}','{$word}');" style="display:none;"/>
		...
		<div class="tableListContainer">
			<table class="table table-hover table-bordered table-striped">
				<tr>
					...
					<th width="5%">S.No</th>
					<th width="{if !$smarty.get.resid}15%{else}15%{/if}" >
					...
					{/if}
					<th width="15%" >
						<a href="javascript:void(0);" {if $smarty.request.sortby eq 'rdesc'}onclick="sortByAscDesc('rasc','{$smarty.get.limit}','{$smarty.request.keyword}');"{elseif $smarty.request.sortby eq 'rasc'}onclick="sortByAscDesc('rdesc','{$smarty.get.limit}','{$smarty.request.keyword}');"{else}onclick="sortByAscDesc('rdesc','{$smarty.get.limit}','{$smarty.request.keyword}');"{/if}>Rating{if $smarty.request.sortby eq 'rdesc'}<img src="images/icon_arrow_down.png" alt="" title="Descending" />{elseif $smarty.request.sortby eq 'rasc'}<img src="images/icon_arrow_up.png" alt="" title="Ascending" />{else}<img src="images/icon_arrow_up.png" alt="" title="Ascending" />{/if}
						</a>
					</th>
				...
				{section name="list" loop=$review_list}
				{assign var="trvar" value=$smarty.section.list.rownum}
				<tr id="deletecate{$review_list[list].rating_id}">
					<td >
						<div class="checkbox checkbox-primary no-margin">
							<input type="checkbox" class="case" value="{$review_list[list].rating_id}" onclick="individualSelect();" id="tableselect_{$smarty.section.list.rownum}" />
							<label for="tableselect_{$smarty.section.list.rownum}"></label>
						</div>
					</td>
					<td >{$review_list[list].sno}</td>
					<td >{$review_list[list].customer_name|stripslashes}</td>
					{if !$smarty.get.resid}
					<td >{$review_list[list].restaurant_name|stripslashes}</td>
					{/if}
					{*<td align="center" class="listCont">{$review_list[list].rating|stripslashes}</td>*}
					<td >
						{if $review_list[list].rating eq '1'} <img alt="1" title="" src="images/single-star.png" /> 
						{elseif $review_list[list].rating eq '2'} <img alt="2" title="" src="images/double-star.png" /> 
						{elseif $review_list[list].rating eq '3'} <img alt="3" title="" src="images/triple-star.png" /> 
						{elseif $review_list[list].rating eq '4'} <img alt="4" title="" src="images/four-star.png" /> 
						{elseif $review_list[list].rating eq '5'} <img alt="5" title="" src="images/five-star.png" /> 
						{/if}
					</td>
					<td >{$review_list[list].message|stripslashes}</td>
                    <td >{$review_list[list].rating_id}</td>
					<td >{$review_list[list].addeddate|date_format:"%d - %m - %Y"}</td>
					<td id="chgstatus{$review_list[list].rating_id}">
						{if $review_list[list].status eq '1'}
						<img src="images/icon_active.png" alt="Active" title="Active" onclick="return changeStatus('0','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$review_list[list].rating_id}','{$filename}');" style="cursor:pointer;" />
						{else}
						<img src="images/delete.png" alt="Deactive" title="Deactive" onclick="return changeStatus('1','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$review_list[list].rating_id}','{$filename}');" style="cursor:pointer;" />
						{/if}
					</td>
                    {if $VIEWABLE eq 'Yes'}
					<td align="center" class="listCont">
						<span class="EditDeleteButton">
							<img src="images/icon_delete.png" width="14" height="14" alt="Delete" title="Delete" onclick="return changeStatus('delete','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$review_list[list].rating_id}','restreviews');" style="cursor:pointer;" />
						</span>
					</td>
					{/if}
				</tr>
