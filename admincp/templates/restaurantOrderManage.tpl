<div class="page-header">
    <h1 class="title">Manage {if $smarty.get.type eq 'CO'}Customer {elseif $smarty.get.type eq 'GO'}Guest {/if} Order {if $smarty.get.resid} - {$restname}{/if}</h1>
      <ol class="breadcrumb">
        <li><a href="index.php">Dashboard</a></li>
        <li class="active">Manage {if $smarty.get.type eq 'CO'}Customer {elseif $smarty.get.type eq 'GO'}Guest {/if} Order {if $smarty.get.resid} - {$restname}{/if}</li>
      </ol>
...
<div class="panel panel-default">
	<div class="panel-body">
		...
		<div class="col-sm-7 no-padding">	
			<form name="ordermanage" method="post" action="restaurantOrderManage.php{if $smarty.get.resid neq ''}?resid={$smarty.get.resid}{/if}{if $smarty.get.type neq ''}?type={$smarty.get.type}{/if}" >
		...
		<div class="manageButtonLeft">
			...
			<div class="fliterbuttonContShow processButton" id="searchkey" style="display:none;">
				<form name="filterform" method="post" action="restaurantOrderManage.php" onsubmit="return filterValidation();">
					...
					<input type="text" name="keyword" id="keyword" value="{$smarty.request.keyword}" class="textboxFilter" placeholder="Order Id/Restaurant Name" />
		...
		<div class="manageButtonLastCont">
			...
			<input type="button" class="btn btn-primary btn-sm but_delete" value="Delete" style="display:none;" onclick="adminActivateDeactivate('delete','deletefield','{$whereField}','{$tablename}','{$word}','Order');" />
		...
		<div class="tableListContainer">
			<table width="100%" border="0" class="table table-hover table-bordered table-striped">
				...
				<tr id="deletecate{$order_list[list].orderid}">
					<td>
						<div class="checkbox checkbox-primary no-margin">
							<input type="checkbox" class="case" value="{$order_list[list].orderid}" id="tableselect_{$smarty.section.list.rownum}" onclick="individualSelect();" />
							<label for="tableselect_{$smarty.section.list.rownum}"></label>
						</div>
					</td>
					<td>{$order_list[list].sno}</td>
					<td>{$order_list[list].ordergenerateid}</td>
					<td>{if $order_list[list].deliverytype eq 'delivery'}Delivery{elseif $order_list[list].deliverytype eq 'pickup'}Pickup{/if}</td>
					
					<td>{*if $order_list[list].deliverytype eq 'delivery'*}{$order_list[list].deliverydate}&nbsp;{if $order_list[list].foodassoonas eq '1'}ASAP{else}{$order_list[list].deliverytime}{/if}{*else}--{/if*}</td>
					...
					<td>{$order_list[list].ordertotalprice|stripslashes}</td>
					<td>{$order_list[list].customercellphone}</td>
					{if !$smarty.get.resid}
					<td>{$order_list[list].restaurant_name|stripslashes}</td>
					{/if}
					<!--{if !$smarty.get.resid}
                    ...
                    <td>{$order_list[list].orderid}</td>
					<td>
					{*{if $order_list[list].status neq 'completed' and $order_list[list].status neq 'failed'}
						<select class="selectpikcer" name="changeorderstatus" onchange="return changeOrderStatus(this.value,'{$fieldname}','{$whereField}','{$tablename}','{$word}','{$order_list[list].orderid}');" data-width="120px">
						...
						{if $order_list[list].status neq 'processing' and $order_list[list].status neq 'failed' and $order_list[list].status neq 'completed' and $order_list[list].status neq 'testing' and $order_list[list].status neq 'preparando' and $order_list[list].status neq 'horneando' and $order_list[list].status neq 'empacando' and $order_list[list].status neq 'listo-para-enviar' and $order_list[list].status neq 'enviado'}
						<select class="selectpicker" name="changeorderstatus" id="changeorderstatus" onchange="return disclaimOrder(this.value,'disclaimReason{$order_list[list].orderid}','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$order_list[list].orderid}');" data-width="120px">
							...
							<option value="processing" {if $order_list[list].status eq 'processing'}selected="selected"{/if} >Accept</option>
							
							<option value="failed" {if $order_list[list].status eq 'failed'} selected="selected"{/if} >Disclaim</option>
                            {*<option value="testing" {if $order_list[list].status eq 'testing'} selected="selected"{/if} >Testing</option>*}
							
							{*<option value="failed" {if $order_list[list].status eq 'failed'} selected="selected"{/if} onclick="$('#disclaimReason{$order_list[list].orderid}').show();">Disclaim</option>*}
						</select>
						<div id="disclaimReason{$order_list[list].orderid}" style="display:none">
							<span class="addPageRightFont">Reason<span class="color-red">*</span></span>				
							<textarea  cols="5" rows="1" style="width:120px;" id="reason{$order_list[list].orderid}"></textarea>
							<input type="button" onclick="return changeOrderStatus('failed','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$order_list[list].orderid}');" style="margin-top:5px;" class="btn btn-default" value="Submit"/>
						
						</div>
						{/if}
						
						{if $order_list[list].status eq 'processing' or $order_list[list].status eq 'preparando' or $order_list[list].status eq 'horneando' or $order_list[list].status eq 'empacando' or $order_list[list].status eq 'listo-para-enviar' or $order_list[list].status eq 'enviado'} 
						
						
						
							{*{if $order_list[list].status neq 'failed' and $order_list[list].status neq 'completed' and $order_list[list].status neq 'testing' and $order_list[list].status neq 'preparando'}*}
						<select class="selectpicker" name="changeorderstatus" onchange="return changeOrderStatus(this.value,'{$fieldname}','{$whereField}','{$tablename}','{$word}','{$order_list[list].orderid}');" data-width="120px">
						
							
							
							<option value="processing" {if $order_list[list].status eq 'processing'}selected="selected"{/if}>Aceptada</option>
							<option value="preparando" {if $order_list[list].status eq 'preparando'}selected="selected"{/if}>Preparando</option>
							<option value="horneando" {if $order_list[list].status eq 'horneando'}selected="selected"{/if}>Horneando</option>
							<option value="empacando" {if $order_list[list].status eq 'empacando'}selected="selected"{/if}>Empacando</option>
							<option value="listo-para-enviar" {if $order_list[list].status eq 'listo-para-enviar'}selected="selected"{/if}>Listo para enviar</option>
							<option value="enviado" {if $order_list[list].status eq 'enviado'}selected="selected"{/if}>Enviada</option>
							<option value="completed" {if $order_list[list].status eq 'completed'}selected="selected"{/if}>Entregada</option>
						...
						Cancelado
						{/if}
                        {if $order_list[list].status eq 'testing'}
						Probando
						{/if}
