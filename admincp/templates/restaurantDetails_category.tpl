{assign var = categoryList value = $objAdminRestaurantMgmt->categoryList($resid)}
	...
	<div class="riteCont1Inner">
		...
		<div class="tableListContainer table-responsive">
			<table  class="table table-hover table-striped table-bordered">
				<tr class="listHeader">
					...
					<th width="65%" align="center" class="">Category</th>
					<th width="15%" align="center" class="">Added Date</th>
					<th width="5%" align="center" class="">Status</th>
				</tr>
				{section name=list loop=$categoryList}
				{assign var="trvar" value=$smarty.section.list.rownum}
				<tr  id="deletecate{$categoryList[list].maincateid}">
					<td align="center" class="">{$categoryList[list].sno}</td>
					<td align="center" class="">{$categoryList[list].maincatename|stripslashes}</td>
					<td align="center" class="">{$categoryList[list].addeddate|date_format}</td>
					<td align="center" class="" id="chgstatus{$categoryList[list].maincateid}">
						{if $categoryList[list].status eq '1'}
				...
				{sectionelse}
				<tr><td colspan="4" align="center" class="text-danger">No record(s) found</td></tr>
				{/section}
			</table>
		</div>
		<!--List End-->
		...
		<div class="manageButtonLastCont">
			{if $limit lt $totalRecords}
				<a class="manageButton_addnw" href="categoryManage.php{if $smarty.get.resid}?searchrestaurantid={$smarty.get.resid}{/if}">View More</a>
			{/if}
			<!--<a class="manageButton_addnw" href="menuAddEdit.php{if $smarty.get.resid neq ''}?resid={$smarty.get.resid}{/if}">Add New</a>
			<input type="button"  class="manageButton but_activate" value="Activate" onclick="adminActivateDeactivate('1','{$fieldname}','{$whereField}','{$tablename}','{$word}');" style="display:none;"/>
			<input type="button" class="manageButton but_deactivate" value="Deactivate" style="display:none;" onclick="adminActivateDeactivate('0','{$fieldname}','{$whereField}','{$tablename}','{$word}');" style="display:none;" />
