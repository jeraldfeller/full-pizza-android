					...
					<tr {if $trvar is even}class="listLightGray"{/if}>
						...
						{*<td align="center" class="listCont">
							{if $cuisine_List[cuisine].cuisine_photo neq ''}
								<div class="largeImgTooltip">
									...
									<span><img src="{$SITE_IMAGE_CUISINE_URL}/{$cuisine_List[cuisine].cuisine_photo|stripslashes}" alt="{$cuisine_List[cuisine].cuisine_name|stripslashes}" title="{$cuisine_List[cuisine].cuisine_name|stripslashes}" width="200" /></span>
								</div>
							{else}
								No Photo
							{/if}
						</td>*}
						<td align="center" class="listCont">{$cuisine_List[cuisine].addeddate}</td>				
						...
						<td align="center" class="">
							<span class="EditDeleteButton">
								<a href="cuisineAddEdit.php?cusid={$cuisine_List[cuisine].cuisine_id}{if $smarty.request.page neq ''}&page={$smarty.request.page}{/if}	{if $smarty.request.limit neq ''}&limit={$smarty.request.limit}{/if}	{if $smarty.request.sortby neq ''}&sortby={$smarty.request.sortby}{/if}	{if $smarty.request.keyword neq ''}&keyword={$smarty.request.keyword}{/if}" class="btn btn-light btn-icon btn-sm" >
									<i class="fa fa-pencil"></i>
								</a>
							</span>
                            <span class="EditDeleteButton">
								<a href="javascript:;" class="btn btn-light btn-sm btn-icon" onclick="return changeStatus('delete','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$cuisine_List[cuisine].cuisine_id}','delete_cuisine');" >
